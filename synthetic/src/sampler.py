...
class DumbbellSampler(DistributedSampler):
    def __init__(
        self, dataset, noniid_percent, num_replicas=None, rank=None, shuffle=True
    ):
    ...
    def __iter__(self):
        ...
        clique1_indices[:exchanged], clique2_indices[:exchanged] = (
            clique2_indices[:exchanged],
            clique1_indices[:exchanged],
        )
        ...
        if self.clique_index == 0:
            ...
            indices = clique2_indices[
                self.rank - self.num_replicas // 2 :: self.num_replicas // 2
            ]
        else:
