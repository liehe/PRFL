...
class CIFAR100Sampler(DistributedSampler):
    """Sampler that restricts data loading to a subset of the dataset.
    ...
    def __init__(self, nlabels_per_cluster, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.nlabels_per_cluster = nlabels_per_cluster

    def __iter__(self):
        nlabels = len(self.dataset.classes)
        assert nlabels == 100
        assert nlabels % self.nlabels_per_cluster == 0
        num_clusters = nlabels // self.nlabels_per_cluster
        assert self.num_replicas % num_clusters == 0
        replicas_per_cluster = self.num_replicas // num_clusters

        # deterministically shuffle based on epoch
        g = torch.Generator()
        g.manual_seed(0)

        # The `indices` is the list of sample indices wose labels are sorted from 0 to 99.
        indices = []
        cluster_indices = []
        for i in range(nlabels):
            indices_i = torch.nonzero(self.dataset.targets == i)
            indices_i = indices_i.flatten().tolist()
            cluster_indices += indices_i

            if i % self.nlabels_per_cluster == self.nlabels_per_cluster - 1:
                # Ensure cluster have expected number of samples
                num_samples_per_cluster = self.num_samples * replicas_per_cluster
                if num_samples_per_cluster > len(cluster_indices):
                    num_to_add = num_samples_per_cluster - len(cluster_indices)
                    cluster_indices = cluster_indices + cluster_indices[:num_to_add]
                else:
                    cluster_indices = cluster_indices[:num_samples_per_cluster]

                # shuffle data within the same cluster such that they are iid
                g = torch.Generator()
                g.manual_seed(i)  # also fixed across epochs
                idx_idx = torch.randperm(len(cluster_indices), generator=g).tolist()
                cluster_indices = [cluster_indices[i] for i in idx_idx]

                # gather data for a new cluster
